define([
	...
	return function(context, imageBuffer, imageScale, fits){
		
		var i, ii;
		var invImageScale = 1 / imageScale;
		for(i=0; i < fits.options.height; i++){
			var x = ( i * invImageScale ) * fits.image.width;
			for(ii=0; ii < fits.options.width; ii++){
				var y = ii * invImageScale;
				var pixel = x+y;
				var index = ((i * fits.options.width) + ii) * 4;
